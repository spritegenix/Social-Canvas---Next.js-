...
export default function Button({
  ...
  const getVariantClasses = () => {
    switch (variant) {
      case "orange":
        return `bg-bg1 text-white hover:bg-bg1/80 ${disabled ? "bg-bg1/80 cursor-not-allowed" : ""}`;
      case "green":
  ...
  return (
    <Component
      className={`${className} ${getVariantClasses()} ${getSizeClasses()} flex items-center justify-center text-nowrap rounded-md text-center capitalize transition-all duration-300 active:scale-90`}
      {...props}
    >
