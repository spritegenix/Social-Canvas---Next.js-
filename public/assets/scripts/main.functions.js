...
$(function () {
  ...
  $(
    'a:not(.lightbox):not(.filter):not(.anchor-link):not([target=_blank]):not([href^="#"])'
  ).addClass('smooth');
  ...
  $html
    .find('header.header .has-dropdown')
  ...
  $window.on('scroll', function () {
    ...
    if (scrollTop > 0) $body.addClass('scrolled scrolled-up');
    else $body.removeClass('scrolled scrolled-up');

    if (scrollTop < lastScrollTop) $body.addClass('scrolled-up');
    // else $body.removeClass('scrolled-up');
